...
blocks = list( numeric = setdiff(colnames(tb2), 'Brand_Code'),
               factor = c('Brand_Code'))

# Specify method for each variable type
imputed_data <- mice(data = tb2, m = 5, method = c("norm", "pmm"), blocks = blocks)

# Final Imputations
df_1 = complete(imputed_data, 1)
df_2 = complete(imputed_data, 2)
df_3 = complete(imputed_data, 3)
df_4 = complete(imputed_data, 4)
df_5 = complete(imputed_data, 5)
```
